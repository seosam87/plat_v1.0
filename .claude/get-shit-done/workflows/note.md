...
- **Global scope**: `/projects/test/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/projects/test/.claude/notes/`)
...
2. Glob `/projects/test/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/projects/test/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/projects/test/.claude/notes/` — works in any directory
