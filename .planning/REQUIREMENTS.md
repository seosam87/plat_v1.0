...
- [ ] **QW-01**: Пользователь видит список страниц с позициями 4–20, у которых есть хотя бы одна нерешённая SEO-проблема (нет TOC, нет schema, мало ссылок, тонкий контент)
- [ ] **QW-02**: Каждая страница в Quick Wins имеет opportunity score = (21 - позиция) x недельный трафик, список отсортирован по score
- [ ] **QW-03**: Пользователь может запустить батч-фикс выбранных страниц (TOC/schema/ссылки) через существующий content pipeline
- [ ] **DEAD-01**: Пользователь видит страницы с 0 визитов за 30 дней (из Метрики) и/или падением позиций > 10 за 30 дней
- [ ] **DEAD-02**: Каждая мёртвая страница имеет рекомендацию: merge, redirect, rewrite или delete — на основе наличия ключей и трафика
...
- [ ] **CPDF-01**: Пользователь может сгенерировать PDF-отчёт для владельца сайта с пошаговыми инструкциями по исправлению проблем
- [ ] **CPDF-02**: Отчёт объединяет Quick Wins + ошибки + рекомендации в понятном формате (проблема -> решение -> шаги в WP admin)
- [ ] **CPDF-03**: Для каждого типа ошибки существует шаблон инструкции на русском языке
